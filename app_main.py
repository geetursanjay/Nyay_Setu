...
if submitted and user_question:
    with st.spinner("Searching for your answer..."):
        ...
        best_match, score, index = process.extractOne(
            user_question, queries, scorer=fuzz.WRatio, score_cutoff=40  # lowered cutoff
        )
        ...
        short_highlighted = highlight_text(short_answer, user_question)
        detailed_highlighted = highlight_text(detailed_answer, user_question)

        # Display answers
        st.markdown(f"**Short Answer:**", unsafe_allow_html=True)
        st.markdown(short_highlighted, unsafe_allow_html=True)
        st.markdown(f"**Detailed Answer:**", unsafe_allow_html=True)
        st.markdown(detailed_highlighted, unsafe_allow_html=True)
